...
bool IsKeyword(const char * mot )
{
   if(current !=KEYWORD)
     return false;
	else
	return strcmp(mot, lexer->YYText())==0;
}
...
{
unsigned long long tag = TagNumber++;
enum TYPES var_boucle;
string keyword;

if(current != KEYWORD)// il a reconnue ca
      {Error("un keyword est attendue ");}

if(strcmp(lexer->YYText(),"FOR")!=0)
       	{Error("c'est pas un for !");}

 current = (TOKEN) lexer->yylex();
 var_boucle=AssignementStatement();

 if(IsKeyword("TO") || IsKeyword("DOWNTO")  ) //comparer le i avec rax
 {
 keyword= lexer->YYText();

 current = (TOKEN) lexer->yylex();
 Expression();

 cout<<"\tTestFor"<<tag<<":"<<endl;
 cout<<"\tmov (%rsp) , %rax \t# Get the result of expression"<<endl;
 cout<< "\tcmp " <<  "%rax ," <<var_boucle<<endl;

 if(keyword == "TO")
 	cout<<"\tjae EndFor"<<tag<<"\t# if i>= resultat de l'expression , jump out of the loop"<<tag<<endl;
 if(keyword == "DOWNTO")
  	cout<<"\tjbe EndFor"<<tag<<"\t# if i<= resultat de l'expression , jump out of the loop"<<tag<<endl;
...
// {
//    // si le suivant est une constante
//   while( current == NUMBER || current == ID){
//   cout<<"\tpop %rax\n";
// 	 cout<<"\tcmpq $"<<lexer->YYText()<<", %rax\n";//on compare le case avec la valuer de base
// 	 string caseTage = GenerateTag("Case");
// 	 cout << "\tje " << caseTage <<"\t# si il y'a une correspondance  en jump "<<endl;
// 	 current = (TOKEN) lexer->yylex();
// 	 if (current == COMMA) {
// 		  	 current = (TOKEN) lexer->yylex();}
...
// 			Statement();
// 		}
// 		cout<< "\tjump EndCase" <<tag<<"\n";
// 		break;
// 	}else{
// 		Error(" attention ',' ou ':' est attendue in case list ");
// 	}
// 	 }}
//
//
...
//     if (Empty()) {
//         current = (TOKEN) lexer->yylex();// Skip empty case list element
//         return;
//     }
// 		//si le current est une liste de cas a traiter
//     CaseLabelList(tag);
//
// }
//
...
//   unsigned long long tag = TagNumber++;
//
// 	if(!IsKeyword("CASE")){Error("mot cle 'CASE' est attendue ");}
//
// 	current = (TOKEN) lexer->yylex();
// 	Expression();
//
// 	if(!IsKeyword("OF"))
// 	{Error("'OF' est attendue ");}
//
// 	current = (TOKEN) lexer->yylex();
// 	cout << "\tjmp TestCase" << tag << "\n";
//
// 	cout << "TestCase" << tag << ":\n";
// 	cout << "\tpop %rax\n";
//
...
// 			 current = (TOKEN) lexer->yylex();
// 			 if (current != COLON) {
// 					 Error("':' est atendue apre le 'ELSE'");
// 			 }
// 			 current = (TOKEN) lexer->yylex();
// 			 Statement();
//
...
//     while (current == NUMBER || current == ID) {
//         cout << "\tpop %rax\n";
//         cout << "\tcmpq $" << lexer->YYText() << ", %rax\n"; // Compare the case with the base value
//         string caseTag = GenerateTag("Case");
//         cout << "\tje " << caseTag << "\t# If there is a match, jump\n";
//         current = lexer_advance();
//         if (current == COMMA) {
//             current = lexer_advance();
//         } else if (current == COLON) {
//             cout << caseTag << ":\n";
...
//                 Statement();
//             }
//             cout << "\tjmp EndCase" << tag << "\n";
//             break;
//         } else {
//             Error("Attention, ',' ou ':' est attendu in case list");
//         }
//     }
// }
//
...
// void CaseListElement(unsigned long long tag) {
//     if (Empty()) {
//         current = lexer_advance(); // Skip empty case list element
//         return;
//     }
//     CaseLabelList(tag);
// }
...
//
// void CaseStatement() {
//     unsigned long long tag = TagNumber++; // Create a unique tag for this case statement
//
//     if (!IsKeyword("CASE")) {
//         Error("Mot clé 'CASE' est attendu");
//     }
//
//     current = lexer_advance();
//     Expression();
...
void CaseStatement() {
    ...
    current = lexer_advance();
    Expression(); // This function should place the result of the expression in %rax
